    ...
    <x-table.table>
        ...
        <x-slot name="content">
            ...
            <x-table.trow type="">
                ...
                <x-table.tdata>{{$form->id}}</x-table.tdata>
                <x-table.tdata>{{$form->title}}</x-table.tdata>
                <x-table.tdata>{{$form->created_at}}</x-table.tdata>
                <x-table.tdata>
                    <x-jet-dropdown align="right" width="60" dropdownClasses="!z-50">
                        ...
                        <x-slot name="content">
                            <div class="">
                                <x-jet-dropdown-link href="{{route('forms.edit',$form->id )}}" class="flex">
                                    <x-icons.pen class="text-xs mr-1 text-yellow-400 font-black" />{{ __('Edit') }}
                                </x-jet-dropdown-link>
                                <x-jet-dropdown-link href="{{route('forms.show',$form->id)}}" class="flex">
                                    <x-icons.eye class="text-xs mr-1 text-blue-400 font-black" />{{ __('Show') }}
                                </x-jet-dropdown-link>
                                <x-jet-dropdown-link wire:click="delete({{$form->id}})" class="flex cursor-pointer">
